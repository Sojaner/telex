...
import { agents, runCli, writeFileConfig, ensureGitignored } from "./agents.js";
 ...
 */
export async function installConfig(bot, flags) {
    const dir = process.cwd();
    const list = agents();
    const entry = { command: "telex", args: ["serve"], ...(bot ? { env: { TELEX_BOT: bot } } : {}) };
    if (flags.scope && flags.scope !== "local" && flags.scope !== "project")
        throw new Error(`--scope takes "local" or "project"`);
    const interactive = !flags.agent && process.stdin.isTTY;
    if (!flags.agent && !interactive)
        throw new Error(`not a terminal — pass --agent <${list.map((a) => a.id).join("|")}> (or --print to just see the config)`);
    ...
    try {
        ...
        if (flags.agent) {
            ...
            if (!flags.yes) {
                ...
                if (answer && !answer.startsWith("y"))
                    return say("Nothing written. cd to the project you want and run it there.");
            }
            say();
            list.forEach((a, i) => say(`  ${String(i + 1).padStart(2)}. ${a.label}`));
            const pick = Number((await rl.question(`\nWhich agent? [1-${list.length}] `)).trim());
            if (!Number.isInteger(pick) || pick < 1 || pick > list.length)
                throw new Error("pick one of the listed numbers");
            agent = list[pick - 1];
        }
        if (agent.cli) {
            ...
            if (!agent.file)
                throw new Error(`${agent.label}'s CLI ("${argv[0]}") is not on PATH, and it has no config file telex can write.`);
            say(`! ${argv[0]} is not on PATH — writing ${agent.file} instead.\n`);
        }
        let local = flags.scope === "local";
        if (agent.localFile && !flags.scope) {
            if (!interactive)
                throw new Error(`${agent.label} needs --scope local (gitignored) or --scope project (committed)`);
        ...
        report(result, agent);
        if (ignored)
            say(`✓ Added ${agent.localFile} to ${ignored}`);
    }
    finally {
        rl?.close();
    }
}
function report(result, agent) {
