...
import { readConfig, writeConfig, configPath, maskToken } from "./config.js";
import { addBotInteractive, installConfig } from "./setup.js";
import { agents, snippet } from "./agents.js";
...
function parseArgsFriendly(options) {
    try {
...
async function run(command, name) {
    switch (command) {
        ...
        case "add": {
            ...
            if (name && config.bots[name])
                throw new Error(`bot "${name}" already exists — use "telex set ${name}"`);
            ...
            if (!flags["chat-id"])
                throw new Error("--token also needs --chat-id");
            if (!name)
                throw new Error("give the bot a name: telex add <name> --token ... --chat-id ...");
        ...
        case "set": {
            ...
            if (flags.token)
                bot.token = flags.token;
            if (flags["chat-id"])
                bot.chatId = numeric(flags["chat-id"]);
            if (flags.allow !== undefined)
                bot.allowFrom = parseAllow(flags.allow);
            if (flags.default)
        ...
        case "remove": {
            ...
            if (config.defaultBot === name)
                config.defaultBot = Object.keys(config.bots)[0];
            writeConfig(config);
        ...
        case "list": {
            ...
            if (!names.length)
                return console.log(`No bots configured. Run "telex add".`);
            if (flags.json)
                return console.log(JSON.stringify(redact(config), null, 2));
        ...
        case "config": {
            ...
            if (flags.json)
                return console.log(JSON.stringify({ mcpServers: { telex: entry } }, null, 2));
            if (flags.print)
                return printConfigs(entry, name);
...
function printConfigs(entry, name) {
    const list = agents();
    console.log(`Agents that install it for you — run in the project root:\n`);
    for (const agent of list.filter((a) => a.cli))
        console.log(`  ${agent.label.padEnd(15)}${agent.cli(name).join(" ")}`);
    ...
    if (name)
        console.log(`Messages from this project default to "${name}"; the agent can still pass another bot.`);
    else
        console.log(`Pass a bot name to pin this project to one: telex config <name>`);
    console.log(`\nIf "telex" is not on PATH, use: "command": "node", "args": ["${serverEntry}"]`);
}
function required(bots, name) {
    if (!name)
        throw new Error("which bot? pass its name");
    const bot = bots[name];
    if (!bot)
        throw new Error(`unknown bot "${name}". Configured: ${Object.keys(bots).join(", ") || "none"}`);
    return bot;
}
...
const numeric = (v) => (/^-?\d+$/.test(v.trim()) ? Number(v.trim()) : v.trim());
function parseAllow(value) {
    ...
    return value.split(",").map((v) => {
        ...
        if (!Number.isInteger(id))
            throw new Error(`--allow takes numeric Telegram user ids, got "${v.trim()}"`);
        return id;
    });
}
const redact = (config) => ({
