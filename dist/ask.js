 ...
 */
export async function receipt(session, chatId, message) {
...
function updateReceipt(session, chatId, message, text) {
    if (message.receipt_id === undefined)
        return;
...
export const markExpired = (session, chatId, messages) => {
    for (const message of messages)
        void updateReceipt(session, chatId, message, RECEIPT.expired);
};
/** Tell the user their message went nowhere, rather than holding it for an agent that may never come. */
export function refuse(session, chatId, message) {
